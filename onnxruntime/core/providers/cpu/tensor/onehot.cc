...
Status OneHotOp<TI, TO>::Compute(OpKernelContext* p_op_kernel_context) const {
  ...
  const int axis = (axis_ == -1) ? indices_num_dims : axis_;
  int64_t prefix_dim_size = 1;
  for (int i = 0; i < axis; ++i) {
    prefix_dim_size *= indices_dims[i];
  }
  TI suffix_dim_size = indices_shape.Size() / prefix_dim_size;
  ...
  Eigen::array<Eigen::DenseIndex, 2> output_dims_eigen{{prefix_dim_size, depth_val, suffix_dim_size}};
  const auto* output_data = output->MutableData<TO>();
  auto output_tensor_eigen = EigenTensorTypes<TO, 3>::EigenTensorMap(output_data, output_dims_eigen);

  auto on_value_eigen = EigenTensorTypes<TO>::Scalar(values->Data<TO>()[1]);
  auto off_value_eigen = EigenTensorTypes<TO>::Scalar(values->Data<TO>()[0]);
